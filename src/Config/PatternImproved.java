...
public class PatternImproved {
	...
	public static void calculatePattern(byte limite){
		...
		patternDFSCoin(src, (byte)1, limite, -1);
		src = new Cube(Cube.src);
		patternDFSEdge(src, (byte)1, limite, -1);
		System.out.println("\n=========== Fin de calcul ===========\n");
		writeBinaryPattern();
		System.out.println("\n=========== Fin d'écriture ===========\n");
	}
	...
	static void patternDFSCoin(Cube c, byte level, byte limite, int lastFace) {
			...
			for (int tour = 0 ; tour < 3 ; tour++)
			{
				...
				a.Run(c);
				{
					...
					if (coin[key] > level)
					{
						coin[key] = level;
						if (level < limite) 
							patternDFSCoin(c, (byte)(level + 1), limite, face);
					}
				}
	...
	static void patternDFSEdge(Cube c, byte level, byte limite, int lastFace) {
		for (int face = 0 ; face < 6 ; face++)
		{
			if (lastFace == face) continue;  //toujours réduire 18 en 15
			for (int tour = 0 ; tour < 3 ; tour++)
			{
				...
				a.Run(c);
				{
					...
					if (level < limite) 
						patternDFSEdge(c, (byte)(level + 1), limite, face);
				}
				a.Rollback(c);  //on retourne en arrière après le parcours
			}
		}
